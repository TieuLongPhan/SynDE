...
from synde import GraphBuilder, assign_pi_systems, normalize_graph


class TestPiSystemAssigner(unittest.TestCase):
    def _assignment(self, smiles: str):
        return assign_pi_systems(GraphBuilder.from_smiles(smiles))
    ...
    def test_pyridine_and_pyrrole_nitrogen_are_distinguished(self) -> None:
        ...
        pyridine_n = next(atom for atom in pyridine.atoms if "nitrogen" in atom.reason)
        pyrrole_n = next(atom for atom in pyrrole.atoms if "nitrogen" in atom.reason)
